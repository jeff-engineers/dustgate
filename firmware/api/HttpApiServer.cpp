...
// WiFiConfig.h provides getHostname()/reset() without pulling in <WebServer.h>,
// which would conflict with ESPAsyncWebServer's HTTP enums.
...
void HttpApiServer::registerRoutes() {
    ...
    // ==================================================================
    // Topology — the persisted shop graph, and the live routing result.
    //
    //   GET /api/topology     → stored JSON verbatim (404 if none yet)
    //   PUT /api/topology     → validate + persist (body accumulated)
    //   GET /api/status       → live routing snapshot (which tool is collecting,
    //                              per-selector state, conflicts, reachability).
    //                              The motion-hardware view is GET /api/motion.
    ...
    // ------------------------------------------------------------------
    _server.on("/api/topology", HTTP_GET, [this](AsyncWebServerRequest* req) {
        if (!checkAuth(req)) return;
    ...
    _server.on("/api/topology", HTTP_PUT,
        [](AsyncWebServerRequest* req) {},
        nullptr,
        [this](AsyncWebServerRequest* req, uint8_t* data, size_t len, size_t index, size_t total) {
            ...
            if (ok) {
                DEBUG_PRINTLN(F("[API] topology saved"));
            ...
            else    { DEBUG_PRINT(F("[API] topology rejected: ")); DEBUG_PRINTLN(err);
    ...
    // task. Same discipline as _lastStatusJson for GET /api/motion.
    ...
    // ------------------------------------------------------------------
    // GET /api/nodes/discover — sweep the LAN for secondary boards.
    //
    // ⚠️ MUST BE REGISTERED BEFORE "/api/nodes". ESPAsyncWebServer matches by
    // PREFIX, not exact path:
    //
    //     _uri != request->url() && !request->url().startsWith(_uri + "/")
    //         → WebHandlerImpl.h, AsyncCallbackWebHandler::canHandle()
    //
    // so "/api/nodes" happily claims "/api/nodes/discover", and handlers
    ...
    // the whole fix. Same trap applies to any future /api/nodes/<x> route.
    //
    ...
    // ------------------------------------------------------------------
    // POST /api/tool  { toolId: "ts", on: true }
    //
    // Switch a tool on/off BY HAND — what the Live view's rows actually do. Until
    // now that view called /api/sim/tool, which only ever existed in the mock
    ...
    // ------------------------------------------------------------------
    // POST /api/nodes/pair  { host: "dustgate-node-1", name?: "Back wall" }
    //                          { host: "…", remove: true }
    //
    ...
    //
    // ⚠️ Registered before "/api/nodes" — prefix matching, see the warning on
    // the discover route above.
    // ------------------------------------------------------------------
    _server.on("/api/nodes/pair", HTTP_POST,
        [](AsyncWebServerRequest* req) {},
        nullptr,
        [this](AsyncWebServerRequest* req, uint8_t* data, size_t len, size_t, size_t) {
            if (!checkAuth(req)) return;
    ...
    // ------------------------------------------------------------------
    // GET /api/nodes — live link state per controller in the topology.
    //
    // Registered AFTER /api/nodes/discover on purpose — see the prefix-match
    ...
    // /api/status: the link objects belong to the loop and the NodeLink task.
    // ------------------------------------------------------------------
    _server.on("/api/nodes", HTTP_GET, [this](AsyncWebServerRequest* req) {
        if (!checkAuth(req)) return;
        xSemaphoreTake(_mutex, portMAX_DELAY);
    ...
    // the angle where the ball actually lines up (see docs/topology-schema.md). That
    ...
    // ------------------------------------------------------------------
    // GET /api/motion — motion-hardware snapshot: stepper state, position,
    // endstops and outlet readings. The same payload the WebSocket pushes; this
    // route exists for one-shot queries that don't want to open a WS.
    // (Routing — which tool is collecting — is GET /api/status.)
    // ------------------------------------------------------------------
    ...
    // firmware.ino. ESP32's mDNS responder is not safe to call from a
    ...
    // (see consumePingRequest() in firmware.ino), not a spawned task —
