...
**Companions:** [`v2-topology-schema.md`](v2-topology-schema.md) ·
[`v2-architecture-rfc.md`](v2-architecture-rfc.md) ·
...
carry ports while the primary keeps the picture ([`v2-architecture-rfc.md`
§6](v2-architecture-rfc.md)).
...
[`v2-architecture-rfc.md` §10](v2-architecture-rfc.md) makes churn in exported
...
1. **The plug's friendly name carries the owner, for humans.** Firmware already
   ...
   ([`ShellyGen2Outlet.cpp:147`](../linear_actuator/outlets/ShellyGen2Outlet.cpp)),
...
collector's `control` ([`v2-topology-schema.md` §element: collector](v2-topology-schema.md)),
...
- `TopologyStore.h` / `TopologyRouter.h` + `/api/v2/topology|status` — per-system
...
- `syncTopologyOutlets` in [`linear_actuator.ino`](../linear_actuator/linear_actuator.ino)
