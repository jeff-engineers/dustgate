...
The original product controlled one rack-and-pinion linear actuator that selected
which of N blast gates is open. The real value that emerged, though, isn't the
actuator — it's the **shop configuration layer and the smart-outlet automation**.
This RFC generalizes the hardware so the same brain + config layer can drive:
...
- **Secondary** nodes: dumb actuator banks. Built as a SEPARATE, ~200-line program
  (`firmware/node/dustgate_node.cpp`), not as `#ifdef`s in the main sketch —
...
- ~~Topology JSON schema specifics~~ — see [`topology-schema.md`](topology-schema.md).
- ~~`NodeLink` protocol versioning + discovery~~ — versioned frames in
  ...
  with a `role` TXT record, surfaced by `GET /api/nodes/discover` and the
