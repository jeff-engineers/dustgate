...
**Companions:** [`architecture-rfc.md`](architecture-rfc.md), [`topology-schema.md`](topology-schema.md)
**Mockups:** four-surface showcase artifact (published from the design session)

The configurator and daily driver. Started as design intent; the three surfaces
(Live view, build canvas, tool-tagging) are now BUILT and browser-verified in demo
mode. The shared model + `/api/*` layer they consume already exist and are tested;
...
the clean presentation-free tree from [`topology-schema.md`](topology-schema.md).
...
- **Identify by power** — the woodworker-friendly trick. To link a
  ...
  1–5 W standby, gray idle (same thresholds as the original outlet configurator,
...
The canvas editor is maybe +100–300 KB of minified JS — it lands in `ffat`,
...
Historical note: these surfaces (`/shop`, `/build`, `/tools`) shipped as lazy
routes hidden behind a host check while they were still WIP. They are now the
whole app — `/` forwards into them and there is nothing else to gate.

1. **Persistence** — the parallel `layout` map (node `x/y` + orientation)
   alongside the topology tree; PUT/GET through the existing `/api/*`.
...
3. **Live view first** — ✅ BUILT (2026-07-30). `live/live.component.ts` on the
   silent lazy route `/shop`; consumes `getTopology` + `getStatus` + `simTool`
...
5. **Tool-tagging pass** — ✅ BUILT (2026-07-30). `tools/tool-setup.component.ts`
   ...
   `drawLevel` tiers (green ≥5 W / amber 1–5 W / gray idle) + `discoverOutlets`,
   ...
   tier only lights on real hardware — the tier logic is the tested original code.

Retire the flat `device-model.js` once the topology surfaces cover everything it did.
...
At the time of writing the ESP32 firmware was still flat (stops+outlets, one linear
stepper, `SmartOutletControl` polling Shelly plugs → a flat `stopIndex`, NVS
config). Topology routing needs a richer layer: multiple selectors (stepper + servo bank),
...
1. **`TopologyRouter` (routing brain)** — ✅ BUILT (2026-07-30).
   `firmware/control/TopologyRouter.h`: a pure, ArduinoJson-based C++ port
...
2. **Topology storage** — ✅ BUILT (2026-07-31).
   `firmware/control/TopologyStore.h` persists `/topology.json` on the
   ...
   `tools/mock-api.js` contract: `GET /api/topology` (verbatim, 404 if none),
   `PUT /api/topology` (chunked-body accumulate → validate → persist), and
   `GET /api/status` (404 until configured, else the `statusView` shape at IDLE
...
4. Retire the flat path once topology routing drives hardware.
...
2. **Ask which way it turns, once.** The servo is often mounted *behind* a wall-mounted
   ...
   (which varies gate to gate), calibration nudges once and asks "did the handle move right?".
...
**Firmware:** new `POST /api/servo/jog` (`{channel, angle}` or `{channel, detach}`),
...
`/gates` covers all three kinds now. `LinearCalibrationComponent` carries the motion
half — manifold profile → home → which end was that →
reference sweep → place each outlet → review — writing `linear.calibration` and
`states[].positionMm` into the topology instead of the flat config. `makeSelector` no
...
`/api/jog`, `/api/setstop`), which address the stepper directly. That's fine: the
schema allows ≤1 linear selector per controller and only the primary board has a stepper
wired, so "the stepper" is unambiguous. A slider on a SECONDARY board would need
endpoints first. Capture writes the device's stop table too, which is what makes Test work
and keeps the motion status view in step with the topology.
...
every view now awaits `api.whenReady()` before its first fetch — a 401 race made the
