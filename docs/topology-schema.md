...
**Companion:** [`architecture-rfc.md`](architecture-rfc.md)
...
`collector.control.outlet` (`syncTopologyOutlets()` in `firmware.ino`). Under a
topology the layout is the source of truth for both, not the flat config's stored
...
## What a linear-only setup looks like in this schema

A lone sliding gate is expressed the same way as anything else: one
