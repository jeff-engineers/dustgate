...
//
//   • the parsed topology document (adopted at boot and on PUT /api/v2/topology)
...
//   no path can close underneath a coasting blower. (v1 got this for free from
//   OUTLET_OFF_DEBOUNCE_MS, which lives in the v1 stop-selection path that a
//   loaded topology bypasses — so v2 had no coast-down at all until this.)
...
// calibration). Surfaced through /api/v2/status so the UI can say which gate.
...
// purpose: every shop wants some, nobody has a UI to set it yet, and v1's
// equivalent slack was 3 s. Matches the schema doc's example.
